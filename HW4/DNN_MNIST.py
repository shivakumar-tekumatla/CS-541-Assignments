...
class DNN:
    ...
    def forward_propagation(self,X,Ws,bs):
        ...
        for i,(w,b) in enumerate(zip(Ws,bs)):
            ...
            # print(w.shape)
            # print(b.shape)
            z = h@w+b#[:,None]  # make sure to transpose b to be a column vector 
            if i != len(Ws)-1:
                h = self.ReLU(z) 
            else:
    ...
    def train_batch(self,X,y,Ws,bs,alpha,epsilon,n,n_,regularize=True):
        # Split the batch 
        X = X[:,n_:n+n_]
        y = y[n_:n+n_]
        # forward propagation 
        y_hat = self.forward_propagation(X,Ws,bs)
    ...
    def tuning(self):
        ...
        for h in self.H: # for each hyper parameter set 
            ...
            no_hidden_units = int(no_hidden_layers)*[int(no_hidden_units)]
            Ws,bs = self.initWeightsAndBiases(int(no_hidden_layers),no_hidden_units)
            # print("shape",self.forward_propagation(X_tr,Ws,bs).shape)
            # bs = arr[:,None]
            # print(Ws[0],bs[0][:,None])
            # input()
            # for i,(w,b) in enumerate(zip(Ws,bs)):
            #     print(i,w,b)
            #     input()
